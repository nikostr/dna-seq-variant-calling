...
rule delly_bcf:
    input:
        ...
        idx=rules.sambamba_markdup.output[1],
    output:
        "results/individual_calls/{sample}.bcf",
    params:
...
rule delly_merge:
    ...
    output:
        sites="results/delly_sites/delly_sites.bcf"
...
rule delly_genotype:
    input:
        ...
        alns=rules.sambamba_markdup.output[0],
    output:
        genotype="results/genotype/{sample}.bcf",
    conda: "../envs/delly.yaml"
    resources:
...
rule delly_bcftools_index:
    ...
    output:
        "results/genotype/{sample}.bcf.csi",
    log:
...
rule delly_bcftools_merge:
    ...
    output:
        "results/merged_genotype/merged_genotype.bcf",
    log:
...
rule delly_copy_number_segmentation:
    ...
    output:
        covfile="results/read_depth_profile/{sample}.cov.gz",
        vcf="results/read_depth_profile/{sample}.vcf",
    conda: "../envs/delly.yaml"
    log:
...
rule segmentation_bed:
    ...
    output:
        bed="results/read_depth_profile/{sample}.bed",
